...
class TestBaseHookComposition:
    ...
    def test_then_callback_exception_does_not_crash(self):
        ...
        with patch(
            "traceml.instrumentation.hooks.ddp_comm_hook.record_event",
...
class TestAutoInstallViaTraceStep:
    ...
    def test_no_auto_install_when_env_disabled(self):
        """TRACEML_NO_AUTO_WRAP_DDP=1 suppresses auto-install."""
        mock_ddp = _make_mock_ddp()
        stack, patches = self._trace_step_patches()
...
class TestDDPUnwrap:
    ...
    def test_maybe_unwrap_ddp_returns_module(self):
        from traceml.sdk.instrumentation import _maybe_unwrap_ddp
...
from traceml.instrumentation.hooks.ddp_comm_hook import install_ddp_comm_hook
from traceml.utils.timing import _STEP_BUFFER
