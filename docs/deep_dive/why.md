...
Pre-2.0 PyTorch was, charitably, a moving target. The dispatcher rewrite shipped incrementally between 1.5 and 1.10. Autograd hook semantics changed visibly across releases. The CUDA caching allocator was retuned multiple times. An instrumentation library built on those internals would have spent more engineering hours on compatibility than on features — the classic instrumentation maintenance treadmill.
